...
async function testAnalyzeFood() {
    ...
    for (const test of testCases) {
        const result = await (0, index_1.analyzeFood)(test.input);
...
async function testLogFood() {
    ...
    const logResult = await (0, index_1.logFood)(userId, nutrition);
    ...
    // Test entry count
    const count = (0, index_1.getUserEntryCount)(userId);
    log(`✅ User has ${count} entry`, count === 1 ? 'green' : 'red');
    // Test empty userId
    log('\n📝 Testing error handling (empty user ID)...', 'cyan');
    const emptyUserResult = await (0, index_1.logFood)('', nutrition);
    log(`${emptyUserResult.success ? '❌' : '✅'} Empty user rejected: ${emptyUserResult.error}`, emptyUserResult.success ? 'red' : 'green');
    // Test empty nutrition
    log('\n📝 Testing error handling (empty nutrition)...', 'cyan');
    const emptyNutritionResult = await (0, index_1.logFood)(userId, { food: '', calories: 0 });
    log(`${emptyNutritionResult.success ? '❌' : '✅'} Empty nutrition rejected: ${emptyNutritionResult.error}`, emptyNutritionResult.success ? 'red' : 'green');
}
/**
...
async function testGetSummary() {
    ...
    for (const food of foods) {
        const analyzed = await (0, index_1.analyzeFood)(food);
        if (analyzed.data) {
            await (0, index_1.logFood)(userId, analyzed.data);
        }
    }
    log(`✅ Added ${foods.length} entries`, 'green');
    // Get summary
    log('\n📊 Getting daily summary...', 'cyan');
    const summaryResult = await (0, index_1.getSummary)(userId);
    if (summaryResult.success && summaryResult.data) {
        ...
        log(`${caloriesMatch ? '✅' : '❌'} Calories total: ${summaryResult.data.totals.calories} (expected ${expectedCalories})`, caloriesMatch ? 'green' : 'red');
    }
    else {
    ...
    const emptyResult = await (0, index_1.getSummary)('newuser999');
...
function testSupportedFoods() {
    ...
    const supported = (0, index_1.getSupportedFoods)();
    log(`✅ ${supported.length} foods supported`, 'green');
    const sampleFoods = ['banana', 'apple', 'egg', 'pizza'];
    for (const food of sampleFoods) {
        const isSupported = (0, index_1.isFoodSupported)(food);
        log(`${isSupported ? '✅' : '❌'} ${food}`, isSupported ? 'green' : 'red');
    }
    // Test unsupported
    const unsupported = (0, index_1.isFoodSupported)('xyz_unknown_food_xyz');
...
function testCustomFood() {
    ...
    (0, index_1.addCustomFood)('burger', customFood);
    const isSupported = (0, index_1.isFoodSupported)('burger');
...
function testStoreConfig() {
    ...
    (0, index_1.updateStoreConfig)({ maxEntriesPerUser: 500 });
...
async function testIntegrationFlow() {
    ...
    const analyzed = await (0, index_1.analyzeFood)('chicken rice');
    if (!analyzed.success) {
    ...
    const logged = await (0, index_1.logFood)(userId, analyzed.data);
    if (!logged.success) {
    ...
    const summary = await (0, index_1.getSummary)(userId);
    if (!summary.success) {
